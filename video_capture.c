...
static int open_camera(struct usb_camera *cam) 
{
	...
	//将buffer分配的足够大，不管是哪种格式的数据都能够保存
	cam->frame_buffer = (unsigned char *) calloc(1, cam->width * cam->height * 3);
...
static int init_camera(struct usb_camera *cam) 
{
	...
	//当设置了错误的分辨率和格式，在VIDIOC_S_FMT后会自动选择一种可用的分辨率和格式，导致前后的不一致
	//为了防止用户发生这种情况，我们把分辨率和格式反馈回用户
